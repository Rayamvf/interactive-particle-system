...
int main() {
    ...
    Image icon = LoadImage("Idmage.png");
    SetWindowIcon(icon);
    UnloadImage(icon);
    ...
    while (!WindowShouldClose()) {
        ...
        if (timeSinceLastSpawn >= particleSpawnTimer) {
            ...
            particles.push_back(CreateParticle({ screenWidth / 4.0f, screenHeight / 2.0f }));
            particles.push_back(CreateParticle({ 3 * screenWidth / 4.0f, screenHeight / 2.0f }));
